<div class="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5"
     x-data="{ displayed: 0, target: {{ $percentage }} }"
    ...
    <div class="flex items-center justify-between mb-3">
        <div class="flex items-center gap-2">
            ...
            @if($percentage === 100)
                <span class="text-xs px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 font-semibold">🎉 Complete</span>
            @endif
        </div>
    ...
    <div class="h-2 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
        <div class="h-full rounded-full transition-all duration-700 ease-out"
             ...
             :style="'width: ' + displayed + '%'"></div>
    </div>

    <p class="text-xs text-zinc-400 mt-2">{{ $completed }} of {{ $total }} lessons completed</p>
